	...
	jQuery.fn.leaveNotice = function(opt){
		...
		var defaults = {
			...
			preLinkMessage: "<div class='setoff'><p>You will now be directed to:<br/>{URL}</p></div>",
                        ...
                        preCancelMessage: "",
                        postCancelMessage: ' or press the ESC key.',
			linkString: "", 
			newWindow: false,
			timeOut: 5000,
		...
		return this.each(function(){
			...
			el.click(function(){
				...
				// }
			    msgContent+='<p id="ln-cancelMessage">';
                            msgContent+=options.preCancelMessage;
                            msgContent+='<a href="#close" id="ln-cancelLink">Cancel</a>';
                            msgContent+=options.postCancelMessage;
                            msgContent+='</p>';
